...
export function OdontogramaPicker({ patients }: { patients: PatientBasic[] }) {
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted">
          Selecciona un paciente para ver o editar su mapa dental interactivo.
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-border bg-surface dark:bg-surface/80">
          <EmptyState icon={Grid3x3} title="Sin pacientes" description="No se encontraron pacientes con ese criterio." />
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-border bg-surface dark:bg-surface/80">
          {filtered.map((p) => (
            <button
              key={p.id}
              onClick={() => router.push(`/odontograma/${p.id}`)}
